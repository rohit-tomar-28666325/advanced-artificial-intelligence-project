...
class ModelEvaluator(BayesNetInference):
    ...
    def __init__(self, configfile_name, datafile_test, prob_query=None):
        ...
        if prob_query is not None:
            ...
            self.normalised_dist = bnu.normalise(self.prob_dist)
            print("%s=%s" % (prob_query, self.normalised_dist))
            ...
            self.performance_matrix = self.compute_performance(true, pred, prob)
    ...
    def get_true_and_predicted_targets(self):
        ...
        for i in range(0, len(self.csv.rv_all_values)):
            ...
            elif target_value == '2': Y_true.append(2)
            elif target_value == 1: Y_true.append(1)
            elif target_value == 0: Y_true.append(0)
            elif target_value == 2: Y_true.append(2)
            ...
            prob_dist = self.get_predictions_from_BayesNet(data_point, self.csv)
            ...
            elif best_key == '2': Y_pred.append(2)
            elif best_key == 1: Y_pred.append(1)
            elif best_key == 0: Y_pred.append(0)
            elif best_key == 2: Y_pred.append(2)
    ...
    def compute_performance(self, Y_true, Y_pred, Y_prob):
        ...
        brier = self.multiclass_brier_score(Y_true, Y_prob)  # Use the custom function for Brier score
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        prob_query = None 
        ModelEvaluator(configfile, datafile_test, prob_query)
