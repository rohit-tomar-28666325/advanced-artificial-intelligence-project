...
class ModelPerfromance:
    ...
    def make_query_discrete(self):
        ...
        for query in self.custom_query:
            ...
            transformed = self.discretizer.transform(query_df)
    ...
    def load_data(self):
        ...
        self.discretizer = KBinsDiscretizer(
            n_bins=4, encode='ordinal', strategy='uniform', subsample=None)
    ...
    def evaluate_perfromance(self):
        ...
        for train_index, test_index in kf.split(self.data):
            ...
            model = hc.estimate(scoring_method=AICScore(
                train_data), max_iter=max_iter)

            self.create_cpt_file_structure(config_file_path, model.edges())

            # create cpt for learned structure
            cpt_obj = CPT_Generator(config_file_path, dataset_train_path)

            model_eval_obj = ModelEvaluator(
                config_file_path, dataset_test_path)
            self.performance_matrix.append({**model_eval_obj.performance_matrix, "training_time": cpt_obj.training_time})
            ...
            print('train_data count', train_data['status'].value_counts(), len(
                train_data['status']))
            print('test_data count', test_data['status'].value_counts(), len(
                test_data['status']))
