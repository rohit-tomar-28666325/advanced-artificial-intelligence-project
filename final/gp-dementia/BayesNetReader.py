...
# Date: 23 October 2024 support for gpytorch Gaussian Proceess regression
...
class BayesNetReader:
    ...
    def load_regression_models(self):
        ...
        # loads the regression_models as per the .pkl/.pth file in the config file
        if  is_regression_models_available:
            try:
                configfile_name = self.bn["regression_models"]
                print("\nLOADING %s ..." % (configfile_name))
                
                if configfile_name.endswith('.pkl'):
                    ...
                    self.bn["scaler"] = regression_models["scaler"]
                    models_file.close()
