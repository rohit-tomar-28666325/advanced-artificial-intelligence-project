...
class BayesNetInference(BayesNetReader):
    ...
    def __init__(self, alg_name, file_name, prob_query, num_samples):
        ...
        if alg_name == 'InferenceByEnumeration':
            ...
            print("unnormalised P(%s)=%s" % (self.query["query"], self.prob_dist[self.query["query_var_val"]]))
            print("normalised P(%s)=%s" % (self.query["query"], normalised_dist[self.query["query_var_val"]]))
    ...
    def enumeration_ask(self):
        ...
        if "regression_models" not in self.bn:
            ...
            Q = {}
            print('self.bn["rv_key_values"]',self.bn["rv_key_values"])
            print('self.query["query_var"]',self.query["query_var"])
...
if __name__ == "__main__":
    ...
    # Assuming you have the label encoders and scaler stored from the preprocessing step
# label_encoders and scaler should be available in your scope

# from sklearn.preprocessing import StandardScaler, LabelEncoder


# def reverse_transform(query_params):
#     # Reverse categorical encoding
#     for col in ['Group', 'M/F', 'Hand']:
#         if col in query_params:
#             query_params[col] = label_encoders[col].inverse_transform([query_params[col]])[0]
...
#             query_params[col] = query_params[col] * stds[i] + means[i]

#     # Reverse discretization for Age
#     if 'Age' in query_params:
#         if query_params['Age'] == 0:
#             query_params['Age'] = (0.5 - 0.5)  # For bin (-inf, -0.5)
#         elif query_params['Age'] == 1:
#             query_params['Age'] = 0.5  # For bin (-0.5, 0.5)
#         else:
#             query_params['Age'] = (float('inf') + 0.5)  # For bin (0.5, inf)

#     # Reverse discretization for MMSE
#     if 'MMSE' in query_params:
#         if query_params['MMSE'] == 0:
#             query_params['MMSE'] = (0.5 - 0.5)  # For bin (-inf, -0.5)
#         elif query_params['MMSE'] == 1:
#             query_params['MMSE'] = 0.5  # For bin (-0.5, 0.5)
#         else:
#             query_params['MMSE'] = (float('inf') + 0.5)  # For bin (0.5, inf)

#     # Reverse discretization for CDR
#     if 'CDR' in query_params:
#         if query_params['CDR'] == 0:
#             query_params['CDR'] = (0.2 - 0.2)  # For bin (-inf, -0.2)
#         elif query_params['CDR'] == 1:
#             query_params['CDR'] = 0.5  # For bin (-0.2, 0.5)
#         else:
#             query_params['CDR'] = (float('inf') + 0.5)  # For bin (0.5, inf)

#     # Reverse discretization for eTIV, nWBV, ASF
#     for col in ['eTIV', 'nWBV', 'ASF']:
#         if col in query_params:
#             # Use quantiles to determine original values based on labels (0, 1, 2)
#             if query_params[col] == 0:
#                 query_params[col] = ...  # Add the logic to get the original value for bin 0
#             elif query_params[col] == 1:
#                 query_params[col] = ...  # Add the logic to get the original value for bin 1
#             else:
#                 query_params[col] = ...  # Add the logic to get the original value for bin 2

#     return query_params

# # Example usage
# query_params = {
#     'Group': 2,  # Encoded value
#     'Visit': 2,
#     'Age': 1,    # Discretized value
#     'EDUC': 14,
#     'SES': 2,
#     'MMSE': 1,   # Discretized value
#     'CDR': 1,    # Discretized value
#     'eTIV': 1,   # Discretized value
#     'nWBV': 0,   # Discretized value
#     'ASF': 1     # Discretized value
# }

# # Reverse the transformations
# original_query = reverse_transform(query_params)
