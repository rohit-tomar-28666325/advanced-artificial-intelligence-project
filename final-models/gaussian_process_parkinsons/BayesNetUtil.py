...
# Version: 1.2, 23 October 2024 support for gpytorch Gaussian Proceess regression
...
def get_probability_given_parents(V, v, evidence, bn):
    ...
    if parents is None and is_gaussian == False:
        ...
        try: # for gpytorch regressors
            model = bn["models_and_likelihoods"][V]["model"]
            likelihood = bn["models_and_likelihoods"][V]["likelihood"]
            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                test_case = torch.Tensor(values).to(bn["device"])
                predictions = likelihood(model(test_case))
                pred_mean = float(predictions.mean.item())
                pred_std = np.sqrt(float(predictions.variance.item()))
                #probability = get_gaussian_density(float(v), pred_mean, bn["stdevs"][V])
                probability = get_gaussian_density(float(v), pred_mean, pred_std)
        
        except Exception: # for sklearn-based regressors
